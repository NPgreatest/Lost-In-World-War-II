...
void BattleWindow::Write_Sql(){
  //  qDebug()<<"begin writeDatagram:......";
    ...
    for(i=0;i<PLAYER_B1_NUMBER;i++){//直接传递数组内容
                    ...
                    if(P1[i].GetSkill().GetSkill()==SkillName::SubWeapon){
                        ...
                        Controller(m1,KEYBOARD_CONTROLL::PFIRE,Key[i],
                                GetAngle(m2-P1[i].GetRect().center().x(),
                                         m3-P1[i].GetRect().center().y()));break;
                    }
                    Controller(m1,KEYBOARD_CONTROLL::PFIRE,Key[i]);break;
                case KEYBOARD_CONTROLL::PCHANGE:
                    Controller(m1,KEYBOARD_CONTROLL::PCHANGE,P1[i].GetHead());break;
                default:break;
                }
                break;
            }
