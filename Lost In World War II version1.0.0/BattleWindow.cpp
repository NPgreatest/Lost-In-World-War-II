...
void BattleWindow::paint(QPainter *painter, QPaintEvent *event){
     ...
     //   QPainter painter(this);//Qpainter
        ...
        for(i=0;i<P1.GetBulletC();i++){
            ...
            painter->drawImage(QRect(P1.GetBulletPos(i).x-10,P1.GetBulletPos(i).y-10,30,30),Shot4_Image);
            }
        }
            if(P1.GetBullet2Alive()==true){
            painter->drawImage(QRect(P1.GetBullet2Pos().x-10,P1.GetBullet2Pos().y-10,30,30),Shot3_Image);
        }
