...
private:
    ...
    QList<Partical_Sql> Partical_List;
    QList<Enemy_Sql> Enemy_List;
    QList<Bullet_Sql> Bullet_List;
    QList<Player_Sql> Player_List;
public:
    Sql();
    ~Sql(){Object_List.~QList();Enemy_List.~QList();Partical_List.~QList();}

    void AddObject(Object_Sql object);
    void AddPartical(Partical_Sql Partical);
    void AddEnemy(Enemy_Sql enemy);
    void AddBullet(Bullet_Sql bullet);
    void AddPlayer(Player_Sql player);
    ...
    Enemy_Sql GetEnemy(int x){return Enemy_List.at(x);}
    Partical_Sql GetPartical(int x){return Partical_List.at(x);}
    Bullet_Sql GetBullet(int x){return Bullet_List.at(x);}
    Player_Sql GetPlayer(int x){return Player_List.at(x);}
    ...
    int GetBullet_Num() const {return Bullet_List.count();}
    int GetPlayer_Num() const {return Player_List.count();}


    void ObjectClear(){Object_List.clear();}
    void PartivalClear(){Partical_List.clear();}
    void EnemyClear(){Enemy_List.clear();}
    void BulletClear(){Bullet_List.clear();}
    void PlayerClear(){Player_List.clear();}
