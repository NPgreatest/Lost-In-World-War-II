...
void BattleWindow::Flash(){//1秒刷新一次
    ...
    if(boss1->GetAlive()==true)boss1->Boss1_ChangeHead(AI1(P1.GetPos(),boss1->GetPos()));//boss AI
     ...
     if(QRandomGenerator::global()->generate()%2==0&&boss1->GetAlive()==true){//boss子弹
         ...
         case UP:t1.y=0;t2={150,boss1->GetPos().y}; CreatePartical(t1,t2,Type::Dialog2);break;
         case DOWN:t2={150,1000};CreatePartical(t1,t2,Type::Dialog2);break;
         case LEFT:t1.x=0;t2={boss1->GetPos().x,150};CreatePartical(t1,t2,Type::Dialog2);break;
         case RIGHT:t2={10000,150};CreatePartical(t1,t2,Type::Dialog2);break;
         }
     }
...
void BattleWindow::Bullet_HitCheck(){
    ...
    if(P1.Player_Hit(boss1->GetBullet3Pos(),boss1->GetBullet3Area())==true&&boss1->GetBullet3Flash()>=20&&boss1->GetAlive()==true){//boss技能判定
        P1.Player_UnderAttack(100);
        CreatePartical(P1.GetPos(),P1.GetArea(),Type::Player_Damage);
        boss1->Boss1_B3_Hit();
    }//Boss子弹命中玩家
