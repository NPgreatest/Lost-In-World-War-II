    ...
    for (i = 0; i < Player_S.count(); i++)
    {
        painter->drawImage(Player_S.at(i).MeRect,
                           P1[this->GetPlayerImage(
                                  Player_S.at(i).number, Player_S.at(i).head, Player_S.at(i).slow)]
                               .GetImage());
    }
    ...
    while (udp_socket->hasPendingDatagrams())
    {
        ...
        while (1)
        {
            ...
            case Message::Load_Object:
                ...
                if (x < 0 || x > X_Max || y < 0 || y > Y_Max || x1 < 1 || y1 < 0 || x1 > X_Max || y1 > Y_Max || type < 1 || type > 11 || m1 < 0 || m1 > 2)
                    break;
            ...
            case Message::Load_Partical:
                ...
                if (ip != 6)
                    break;
                in >> x >> y >> x1 >> y1 >> ptype;
                if (x < 0 || x > X_Max || y < 0 || y > Y_Max || x1 < 1 || y1 < 0 || x1 > X_Max || y1 > Y_Max)
                    break;
            ...
            case Message::Load_Enemy:
                ...
                if (x < 0 || x > X_Max || y < 0 || y > Y_Max || x1 < 1 || y1 < 0 || x1 > X_Max || y1 > Y_Max || type < 1 || type > 4 || head < 0 || head > 3)
                    break;
                Enemy_S.append(Enemy_Sql(QRect(x, y, x1, y1), head, type));
                break;
            case Message::Load_Bullet:
                in >> count;
                if (count > PLAYER_B1_NUMBER || count < 0)
                    break;
                in >> x >> y >> x1 >> y1 >> type;
                if (x < -100 || x > X_Max || y < -100 || y > Y_Max || x1 < 1 || y1 < 1 || x1 > 200 || y1 > 200 || type < 0 || type > 4)
                    break;
            ...
            case Message::Dead_Bullet:
                in >> ip;
                if (bullet1[count].GetAlive())
                    bullet1[count].PlayerBullet1_Dead();
                break;
            case Message::Load_Bullet2:
                in >> count;
                if (count > PLAYER_B2_NUMBER || count < 0)
                    break;
                in >> x >> y >> x1 >> y1 >> type;
                if (x < -100 || x > X_Max || y < -100 || y > Y_Max || x1 < 1 || y1 < 1 || x1 > 200 || y1 > 200 || type < 0 || type > 4)
                    break;
            ...
            case Message::Load_Player:
                ...
                if (x < 0 || x > X_Max || y < 0 || y > Y_Max || x1 < 1 || y1 < 0 || x1 > 50 || y1 > 50 || type < 0 || type > 10 || head < 0 || head > 3)
                    break;
...
void Client::keyPressEvent(QKeyEvent *event)
{
    if (!event->isAutoRepeat())
    {
        ...
        { //wsad space
        case 87:
            output << Message::KEYBOARD_PUSHD << this->MeNumber << HEAD::UP;
            head = UP;
            break;
        case 83:
            output << Message::KEYBOARD_PUSHD << this->MeNumber << HEAD::DOWN;
            head = DOWN;
            break;
        case 65:
            output << Message::KEYBOARD_PUSHD << this->MeNumber << HEAD::LEFT;
            head = LEFT;
            break;
        case 68:
            output << Message::KEYBOARD_PUSHD << this->MeNumber << HEAD::RIGHT;
            head = RIGHT;
            break; //1代表按下 0123代表
        case 32:
            output << Message::KEYBOARD_OTHER << this->MeNumber << KEYBOARD_CONTROLL::PFIRE << Mouse_Point.x() << Mouse_Point.y();
            break; //2代表发炮
        case 16777248:
            output << Message::KEYBOARD_OTHER << this->MeNumber << KEYBOARD_CONTROLL::PCHANGE;
            break; //换武器技能
        }
...
void Client::keyReleaseEvent(QKeyEvent *event)
{
    if (!event->isAutoRepeat())
    {
        ...
        { //wsad space
        case 87:
            output << Message::KEYBOARD_RELEASED << this->MeNumber;
            break;
        case 83:
            output << Message::KEYBOARD_RELEASED << this->MeNumber;
            break;
