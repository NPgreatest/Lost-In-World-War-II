...
{ //1秒刷新一次
    ...
    for (i = 0; i < PLAYER_NUMBER_MAX; i++)
    {
        ...
        if (P1[i].GetProcess() < 100)
            FinalWeapon.start(100);
    }

    allmusic.PlayBackMusic(settings.backsound / 2);
    ...
    { //物体动画
        ...
        for (j = 0; j < PLAYER_NUMBER_MAX; j++)
        {
            if (!P1[j].GetActivate())
                continue;
            if (object[i]->GetType() == 10 && P1[j].GetRect().intersects(object[i]->GetRect()) && object[i]->GetOther() == 1)
            { //地刺
    ...
    if (boss1->GetAlive() && boss1->GetBullet3Flash() == 0)
        boss1->Boss1_ChangeHead(AI1(P1[0].GetRect(), boss1->GetRect())); //boss AI

    if (QRandomGenerator::global()->generate() % 2 == 0 && boss1->GetAlive() && boss1->GetB3().GetFlash() == 0)
    { //boss子弹
        boss1->Boss1_B3Fire();
        QRect temp;
        ...
        {
        case UP:
            temp.setRect(boss1->GetRect().x(), 0, 150, boss1->GetRect().y());
            CreatePartical(temp, Type::Boss_Fire_D);
            break;
        case DOWN:
            temp.setRect(boss1->GetRect().x(), boss1->GetRect().y(), 150, 1000 - boss1->GetRect().y());
            CreatePartical(temp, Type::Boss_Fire_D);
            break;
        case LEFT:
            temp.setRect(0, boss1->GetRect().y(), boss1->GetRect().x(), 150);
            CreatePartical(temp, Type::Boss_Fire);
            break;
        case RIGHT:
            temp.setRect(boss1->GetRect().x() + 150, boss1->GetRect().y(), 2000 - boss1->GetRect().y(), 150);
            CreatePartical(temp, Type::Boss_Fire);
            break;
        case NO:
            break;
    ...
    for (i = 0; i < ENEMY_NUMBER; i++)
        if (enemy[i].GetAlive())
            Win = false;
    for (i = 0; i < OBJECT_NUMBER; i++)
        if (object[i]->GetAlive() && object[i]->GetType() == 5)
            Win = false;
    //判断敌人是否全部被歼灭
...
{ //毫秒Tick
    ...
    { //轰炸区动画
        if (redzone[i].GetAlive() == false)
            continue;
        if (redzone[i].GetFlash() == -1)
            continue;
        if (redzone[i].GetFlash() == 70)
            RedZoneBoom(i);
        redzone[i].Flash();
    }
...
{ //子弹
    ...
    for (i = 0; i < PLAYER_B1_NUMBER; i++)
    {
        if (!bullet1[i].GetAlive())
            continue;
        bullet1[i].PlayerBullet1_Move();
    }
    for (i = 0; i < PLAYER_B2_NUMBER; i++)
    {
        if (!bullet2[i].GetAlive())
            continue;
        bullet2[i].PlayerBullet2_Move();
    }
    Bullet_HitCheck();
}
...
{
    bool slow = false;
    if (P1[k].GetRect().x() < -500 || P1[k].GetRect().y() > Y_Max + 500 || P1[k].GetRect().y() < -500 || P1[k].GetRect().x() - 500 > X_Max)
        return;
    int i;
    if (P1[k].GetRect().x() < 0 || P1[k].GetRect().y() + 40 > Y_Max || P1[k].GetRect().y() < 0 || P1[k].GetRect().x() + 40 > X_Max)
    { //玩家边界判断
        P1[k].Player_Move(-3);
    ...
    { //墙判断
        ...
        if (P1[k].GetRect().intersects(object[i]->GetRect()) && object[i]->Contact())
        {
            ...
            {
            case 4:
                MissionRougueLike_Win();
                break; //传送门功能
            case 6:
                switch (object[i]->GetSkin())
                {
                case 1:
                    P1[k].Strengthen(2, 10, 0, 0);
                    return;
                case 2:
                    P1[k].Strengthen(10, 10, 0, 0);
                    return;
                case 3:
                    P1[k].Strengthen(50, 50, 0, 0);
                    return;
                }
                break;
            case 7:
                ...
                {
                case 1:
                    P1[k].Strengthen(0, 100, 0, 0);
                    return;
                case 2:
                    P1[k].Strengthen(0, 200, 0, 0);
                    return;
                case 3:
                    P1[k].Strengthen(0, 300, 1, 0);
                    return;
                }
                break;
            case 11:
...
{
    if (boss1->GetAlive() == false || boss1->GetBullet3Flash() != 0)
        return;
    int i;
    bool hit = false;
    boss1->Boss1_Move(5);
...
void BattleWindow::Enemy_Move()
{
    ...
    a:
        if (enemy[i].GetRect().x() < 0 || enemy[i].GetRect().x() + enemy[i].GetRect().width() > X_Max || //边界碰撞
            enemy[i].GetRect().y() < 0 || enemy[i].GetRect().y() + enemy[i].GetRect().height() > Y_Max)
        { //边界碰撞
        ...
        { //物体碰撞检测 k代表物体，i代表敌人
            if (object[k]->GetAlive() == false ||
                object[k]->GetType() == 5 || object[k]->GetType() == 6 || object[k]->GetType() == 7)
                continue;
        b:
            if (enemy[i].GetRect().intersects(object[k]->GetRect()) && object[k]->Contact())
            {
                hit = true;
        ...
        if (!ice)
            enemy[i].Enemy_UnSlow(); //效果实现
        if (hit && !ice)
            enemy[i].Enemy_RndChangeHead(); //效果实现
    }
...
{ //兵营生成敌人
    ...
    for (i = 0; i < OBJECT_NUMBER; i++)
    {
        if (object[i]->GetType() != 5 || object[i]->GetAlive() == false)
            continue;
        ...
        t1 = object[i]->GetRect();
        if (x % 2 == 1)
            t1.adjust(-50, 0, -50, 0); //随机生成位置
        else
            t1.adjust(50, 0, 50, 0);
        if (y % 2 == 1)
            t1.adjust(0, 50, 0, 50);
        else
            t1.adjust(0, -50, 0, -50); //随机生成位置
...
{ //敌人转向和开火
    ...
    for (i = 0; i < ENEMY_NUMBER; i++)
    {
        ...
        case 1:
            if (x % 4 == 0)
                CreateBullet(enemy[i].GetRect().adjusted(enemy[i].GetRect().width() / 2 - 5,
                                                         enemy[i].GetRect().height() / 2 - 5,
                                                         -enemy[i].GetRect().width() / 2 + 5,
                                                         -enemy[i].GetRect().height() / 2 + 5),
                             enemy[i].GetHead(),
                             1, 1, Target::PLAYER, -1, enemy[i].GetB1Speed());
            break;
        case 2:
            if (x % 10 == 0)
                CreateBullet(enemy[i].GetRect().adjusted(enemy[i].GetRect().width() / 2 - 5,
                                                         ...
                                                         -enemy[i].GetRect().height() / 2 + 5),
                             enemy[i].GetHead(),
                             2, 1, Target::PLAYER, -1, enemy[i].GetB1Speed());
            break;
        case 3:
            if (x % 10 == 0)
                CreateBullet(enemy[i].GetRect().adjusted(enemy[i].GetRect().width() / 2 - 5,
                                                         ...
                                                         -enemy[i].GetRect().height() / 2 + 5),
                             enemy[i].GetHead(),
                             3, 1, Target::PLAYER, -1, enemy[i].GetB1Speed());
            break;
        case 4:
            if (x % 10 == 0)
                CreateBullet(enemy[i].GetRect().adjusted(enemy[i].GetRect().width() / 2 - 5,
                                                         ...
                                                         -enemy[i].GetRect().height() / 2 + 5),
                             enemy[i].GetHead(),
                             4, 1, Target::PLAYER, -1, enemy[i].GetB1Speed());
            break;
        case 5:
            if (x % 2 == 0)
                CreateBullet(enemy[i].GetRect().adjusted(enemy[i].GetRect().width() / 2 - 5,
                                                         ...
                                                         -enemy[i].GetRect().height() / 2 + 5),
                             enemy[i].GetHead(),
                             5, 1, Target::PLAYER, -1, enemy[i].GetB1Speed());
            break;
        }
    }
    x = QRandomGenerator::global()->generate();
    if (boss1->GetAlive() && x % 10 == 0)
    {
        ...
        for (i = 0; i < Edit_Min; i++)
        {
            ...
            QRect temp;
            temp = boss1->GetRect();
            temp.adjust(50, 50, -50, -50);
            CreateBullet2(temp, x * 3.14159 / 6, 3, 2, Target::PLAYER, -1, 5, 2);
            x++;
        }
    }
...
void BattleWindow::Mist_Generate()
{
    ...
    {
        if (!P1[k].GetActivate() || P1[k].GetChoose() == 1)
            continue;
        int i;
        for (i = 0; i < 5; i++)
        {
