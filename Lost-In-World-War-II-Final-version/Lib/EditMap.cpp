...
EditMap::EditMap(QWidget *parent) : QMainWindow(parent),
                                    ui(new Ui::EditMap)
{
    ui->setupUi(this);
...
void EditMap::mousePressEvent(QMouseEvent *ev)
{
    ...
    if (ev->button() != Qt::LeftButton || ev->type() != QEvent::Type::MouseButtonPress)
        return; //只响应鼠标左键的单击操作 防止游戏结束重复弹框
    QPoint pt = ev->pos();
    ...
    { //非精确模式
        ...
        {
            if (Rect[i].x() == x && Rect[i].y() == y && Rect[i].width() == x1 && Rect[i].height() == y1 && type[i] == Ctype)
                return; //重复就退出
        }
        if (Ctype == 101)
        {
            for (i = 0; i < Edit_Max; i++)
            {
                if (E_Rect[i].x() == x && E_Rect[i].y() == y && E_Rect[i].width() == x1 && E_Rect[i].height() == y1)
                    return; //重复就退出
            }
        }
        switch (Ctype)
    ...
    { //acc==true//设置反了懒得改
        ...
        {
        case 1:
            thp = QInputDialog::getInt(this, tr("矫正数值"), tr("请输入血量"), 1, 0, 1000, 1, nullptr, nullptr);
            break;
        case 5:
            thp = QInputDialog::getInt(this, tr("矫正数值"), tr("请输入血量"), 10, 0, 1000, 1, nullptr, nullptr);
            break;
        case 8:
            thp = QInputDialog::getInt(this, tr("矫正数值"), tr("请输入血量"), 50, 0, 1000, 1, nullptr, nullptr);
            break;
        case 11:
            thp = QInputDialog::getInt(this, tr("矫正数值"), tr("请输入血量"), 100, 0, 1000, 1, nullptr, nullptr);
            break;
        case 102:
            ppos.setX(x);
            ppos.setY(y);
...
void EditMap::paintEvent(QPaintEvent *event)
{
    ...
    if (me)
        painter.drawImage(QRect(ppos.x(), ppos.y(), 40, 40), number[102]);
...
void EditMap::on_Finish_clicked()
{
    ...
    {
        QMessageBox::information(nullptr, tr("抱歉"), tr("未知错误"), tr("好"));
        return;
    }
