...
void BattleWindow::Bullet_HitCheck()
{
    ...
    for (i = 0; i < PLAYER_B1_NUMBER; i++)
    {
        ...
        {
            if (!object[k]->GetAlive() || (bullet1[i].GetMaster() == -1 && object[k]->GetType() == 5))
                continue; //敌人不会打兵营
        ...
        case Target::ENEMY:
            ...
            { //子弹打中敌人
                ...
                if (bullet1[i].GetRect().intersects(enemy[k].GetRect()))
                {
                    ...
                    if (!enemy[k].GetAlive())
                        CreatePartical(enemy[k].GetRect().adjusted(-50, -50, 100, 100), Type::Object_Hit);
                    else
                    {
                        CreatePartical(enemy[k].GetRect(), Type::Object_Hit);
                    }
                }
            }
            ...
            if (bullet1[i].GetTarget() == Target::ENEMY)
                break;
        case Target::PLAYER:
            for (k = 0; k < PLAYER_NUMBER_MAX; k++)
            {
                if (!P1[k].GetActivate() || k == bullet1[i].GetMaster())
                    continue; //玩家友伤不会命中自己
            ...
            if (bullet1[i].GetTarget() == Target::PLAYER)
                break;
        }
        //玩家和敌人判定
    }
    ...
    for (i = 0; i < PLAYER_B2_NUMBER; i++)
    {
        ...
        for (k = 0; k < OBJECT_NUMBER; k++)
        {
            if (!object[k]->GetAlive() || (bullet2[i].GetMaster() == 100 && object[k]->GetType() == 5))
                continue; //敌人不会打兵营
        ...
        case Target::ENEMY:
            ...
            if (bullet2[i].GetTarget() == Target::ENEMY)
                break;
        case Target::PLAYER:
            for (k = 0; k < PLAYER_NUMBER_MAX; k++)
            {
                if (!P1[k].GetActivate() || k == bullet2[i].GetMaster())
                    continue; //玩家友伤不会命中自己
            ...
            if (bullet2[i].GetTarget() == Target::PLAYER)
                break;
        }
        //玩家和敌人判定
    }
    ...
    for (i = 0; i < PLAYER_NUMBER_MAX; i++)
    {
        ...
        if (B3[i])
            P1[i].GetB3().PlayerBullet3_Dead();
        if (P1[i].GetRect().intersects(boss1->GetB3().GetRect()) && boss1->GetB3().GetFlash() >= 50 && boss1->GetAlive())
        { //boss技能判定
