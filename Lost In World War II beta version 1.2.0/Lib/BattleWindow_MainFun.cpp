...
void BattleWindow::Write_Sql(){
    ...
    /*for(i=0;i<sql.GetObject_Num();i++){
        for(k=0;k<OBJECT_NUMBER;k++){
            if(sql.GetObject(i).MeRect==object[i]->GetRect()&&sql.GetObject(i).MeImage==object[i]->GetImage()){
                sql.PoPObject(i);
            }
        }||sql.GetIsObject(Object_Sql(object[i]->GetRect(),object[i]->GetImage()))
    }*/
    sql.ObjectClear();
    sql.EnemyClear();
    sql.PartivalClear();
    ...
    for(i=0;i<OBJECT_NUMBER;i++){
        if(!object[i]->GetAlive()) continue;
        sql.AddObject(Object_Sql(object[i]->GetRect(),object[i]->GetImage()));
    }
    for(i=0;i<PARTICAL_NUMBER;i++){
        if(!partical[i].GetAlive()) continue;
    ...
    for(i=0;i<sql.GetObject_Num();i++){
        out<<"\nWrite Object :"<<i<<"\n";
        out<<"Object_Rect:\n";
        out<<"x:"<<sql.GetObjectW(i).MeRect.x()<<"\n";
        out<<"y:"<<sql.GetObjectW(i).MeRect.y()<<"\n";
        out<<"width:"<<sql.GetObjectW(i).MeRect.width()<<"\n";
        out<<"height:"<<sql.GetObjectW(i).MeRect.height()<<"\n";
        out<<"MeImgae:"<<sql.GetObjectW(i).MeImage.text()<<"\n";
        out<<"Write Object "<<i<<" finished...\n\n";
    }
