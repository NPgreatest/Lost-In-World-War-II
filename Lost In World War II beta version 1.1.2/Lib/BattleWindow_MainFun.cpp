...
void BattleWindow::Write_Sql(){
    ...
    for(i=0;i<OBJECT_NUMBER;i++){
        //bool del=false;
        if(!object[i]->GetAlive()) continue;
        //if(!sql.GetIsObject(Object_Sql(object[i]->GetRect(),object[i]->GetType())))
        sql.AddObject(Object_Sql(object[i]->GetRect(),object[i]->GetType()));

       // for(k=0;k<sql.GetObject_Num();i++){
       //     if(sql.GetObjectW(k).MeRect==object[i]->GetRect()&&sql.GetObjectW(k).type==object[i]->GetType()&&!object[i]->GetAlive())
       //         del=true;break;
        //}
       // if(del){
        //    sql.PoPObject(sql.GetObjectW(k));
        //}
    }
//qDebug()<<sql.GetObject_Num();

    //for(i=0;i<PARTICAL_NUMBER;i++){
    //    if(!partical[i].GetAlive()) continue;
     //   sql.AddPartical(Object_Sql(partical[i].GetRect(),partical[i].GetImage()));
    //}
    /*for(i=0;i<ENEMY_NUMBER;i++){
        if(!enemy[i].GetAlive()) continue;
        QRect t1[ENEMY_BULLET_NEMBER];
        QImage t2[ENEMY_BULLET_NEMBER];
        for(k=0;k<ENEMY_BULLET_NEMBER;k++){
            t1[k]=enemy[i].GetB1(k).GetRect();
            t2[k]=enemy[i].GetB1(k).GetImage();
        }
        sql.AddEnemy(Enemy_Sql(enemy[i].GetRect(),enemy[i].GetImage(),t1,t2));
    }*/
    ...
    //output << (qint32)sql.GetObject_Num() ;
    //qDebug()<<(qint32)sql.GetObject_Num();
    ...
    for(i=0;i<sql.GetObject_Num();i++){
        ...
        //qDebug()<<sql.GetObjectW(i).MeRect.x()<<sql.GetObjectW(i).MeRect.y()<<sql.GetObjectW(i).MeRect.width()<<sql.GetObjectW(i).MeRect.height();
    }
    ...
    /*for(i=0;i<sql.GetEnemy_Num();i++){
        out<<"\nWrite Enemy and its Bullet:"<<i<<"\n";
        out<<"Enemy_Rect:\n";
        out<<"x:"<<sql.GetEnemey(i).MeRect.x()<<"\n";
        out<<"y:"<<sql.GetEnemey(i).MeRect.y()<<"\n";
        out<<"width:"<<sql.GetEnemey(i).MeRect.width()<<"\n";
        out<<"height:"<<sql.GetEnemey(i).MeRect.height()<<"\n";
        out<<"MeImgae:"<<sql.GetEnemey(i).MeImage.text()<<"\n";
        out<<"\nWrite Enemy bullet:\n";
        for(k=0;k<ENEMY_BULLET_NEMBER;k++){
            out<<"the "<<k<<"Bullet:\n";
            out<<"x:"<<sql.GetEnemey(i).B1Rect[k].x()<<"\n";
            out<<"y:"<<sql.GetEnemey(i).B1Rect[k].y()<<"\n";
            out<<"width:"<<sql.GetEnemey(i).B1Rect[k].width()<<"\n";
            out<<"height:"<<sql.GetEnemey(i).B1Rect[k].height()<<"\n";
            out<<"Dont wanna write MeImage";
            out<<"Write Bullet"<<k<<"Complete:\n\n";
        }
    }*/
    /*for(i=0;i<sql.GetPartical_Num();i++){
        out<<"\nWrite Partical :"<<i<<"\n";
        out<<"Partical_Rect:\n";
        out<<"x:"<<sql.GetPartical(i).MeRect.x()<<"\n";
        out<<"y:"<<sql.GetPartical(i).MeRect.y()<<"\n";
        out<<"width:"<<sql.GetPartical(i).MeRect.width()<<"\n";
        out<<"height:"<<sql.GetPartical(i).MeRect.height()<<"\n";
        out<<"MeImgae:"<<sql.GetPartical(i).MeImage.text()<<"\n";
        out<<"Write Partical "<<i<<" finished...\n\n";
    }*/
  ...
  /*  int i;
    ...
    while (udp_socket->hasPendingDatagrams())
    {
        ...
        in >>str1>>str2>>str3;
            ...
            in >> n;
            qDebug()<<"头："<<str1<<str2<<"+"<<n;
            for(i=0;i<(int)n;i++){
                in>>x>>y>>x1>>y1>>type;
                qDebug()<<"消息："<<x<<y<<x1<<y1<<type;
            }
