...
void Boss1::Boss1_B3Fire(){
    if(B3.GetAlive()) return;
    QRect temp;
    switch (this->head) {
        case UP:temp.setRect(MeRect.x(),0,150,MeRect.y());B3.Init(temp,70);break;
        case DOWN:temp.setRect(MeRect.x(),MeRect.y(),150,1000-MeRect.y());B3.Init(temp,70);break;
        case LEFT:temp.setRect(0,MeRect.y(),MeRect.x(),150);B3.Init(temp,70);break;
        case RIGHT:temp.setRect(MeRect.x()+150,MeRect.y(),2000-MeRect.y(),150);B3.Init(temp,70);break;
    }
    return;
}
...
void Boss1::Boss1_B2Fire(){
    ...
    if(!B2[i].GetAlive()){
        ...
        B2[i].Init(temp,k*3.14159/6,3,2);
        k++;
    }
    }
