...
void BattleWindow::MissionTick(int Mission){
    ...
    case 1:
    {
        if(Total_Time==11){
            ...
            P1.SetPos(QRect(50,450,40,40));
            enemyall.start(100);

        }
        if(P1.GetRect().x()<350&&P1.GetRect().y()<400&&Process==1){
            Process++;
            CreatePartical(QRect(0,500,1500,500),Type::Dialog3);
        }
        if(P1.GetRect().x()<350&&P1.GetRect().y()>550&&Process==2){
            Process++;
            CreatePartical(QRect(0,500,1500,500),Type::Dialog4);
        }
        if(P1.GetRect().x()>950&&Process==3){
            Process++;
...
void BattleWindow::MissionTechEnd(){
    enemyall.stop();
    P1.SetPos(QRect(-1000,-1000,40,40));
    LevelB=4;
    Total_Time=0;
    Process=1;
